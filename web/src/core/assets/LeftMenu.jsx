    ...
    <Grid.Column id="sidebar">
      <Link to="/dashboard" style={{ display: 'block' }}>
        <div id="software-name">
          <div style={{ marginTop: "2px", color: "#355FB3", fontSize: "35px", fontWeight: '1200' }}>HEAL-APP</div>
          <div style={{ marginTop: "2px", marginBottom: "15px", color: "#122240", opacity: "70%", fontSize: "20px" }}>Fitness Management</div>
      ...
      <Menu vertical borderless fluid text>
        {LeftMenuItems.map((item) => (
          <div key={item.key}>
            <Menu.Item
              style={{
                backgroundColor: activeItems.includes(item.key) ? '#dbacac' : '#f0eeee',
                fontWeight: "600",
                borderRadius: "5px",
                marginLeft: '5px',
                marginRight: '5px',
              ...
              onMouseEnter={(e) => (e.target.style.backgroundColor = '#dbacac')} // Chuyển màu khi hover
              onMouseLeave={(e) => (e.target.style.backgroundColor = activeItems.includes(item.key) ? '#dbacac' : '#f0eeee')} // Quay lại màu ban đầu khi rời chuột
            >
              ...
              >
                {item.subItems.map((subItem) => (
                  <Link to={subItem.link} key={subItem.key}>
                    <Menu.Item
                      style={{
                        backgroundColor: activeItems.includes(subItem.key) ? '#dbacac' : '#f0eeee',
